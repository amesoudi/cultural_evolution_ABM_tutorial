...
for (t in 2:t_max) {
  ...
  previous_agent <- agent  # copy agent dataframe to previous_agent dataframe
  
  agent <- data.frame(trait = sample(previous_agent$trait, N, replace = TRUE), stringsAsFactors = FALSE)  # randomly copy from previous generation's agents
  
  output$p[t] <- sum(agent$trait == "A") / N  # get p and put it into the output slot for this generation t
...
plot(output$p, type = 'l', ylab = "p, proportion of agents with trait A", xlab = "generation", ylim = c(0,1), main = paste("N =", N))
...
UnbiasedTransmission <- function (N, t_max, r_max) {
  ...
  for (r in 1:r_max) {
    ...
    agent <- data.frame(trait = sample(c("A","B"), N, replace = TRUE), stringsAsFactors = FALSE)  # create first generation
  ...
  plot(rowMeans(output), type = 'l', ylab = "p, proportion of agents with trait A", xlab = "generation", ylim = c(0,1), lwd = 3, main = paste("N =", N))
