...
plot(x, dgumbel(x, a, beta), type = 'l', ylab = expression("Probability Imitator Acquires "*italic("z"[i])), xlab = expression("Imitator "*italic("z"[i])*" Value"), bty='l')

# add a vertical dotted line for the demonstrator with z_h
alpha <- 30
abline(v = a + alpha, lty=2)
text(a + alpha + 17, 0.02, labels = expression("Demonstrator "*italic("z"[h])*" value"))

# add beta label and arrows
text(a+2, 0.013, labels = expression(beta))
arrows(a+4, 0.013, a+19, 0.013, length=0.1)
arrows(a-1, 0.013, a-13, 0.013, length=0.1)

# add alpha label and arrows
text(a+alpha/2, 0.025, labels = expression(alpha))
...
DemographyModel <- function(N, alpha, beta, t_max) {
  ...
  output <- data.frame(z_bar = rep(NA, t_max)) # create output dataframe to hold mean trait value for each generation
  
  agent <- data.frame(trait = rgumbel(N, beta = beta))  # create first generation, N random draws from a gumbel with mode a=0 and dispersion beta
  ...
  for (t in 2:t_max) {
    ...
    z_h <- max(agent$trait)  # get highest skill from current generation
    
    agent$trait <- rgumbel(N, a = z_h - alpha, beta = beta)  # draw new values
    
    output$z_bar[t] <- sum(agent$trait) / N  # store new mean trait value
    
  }
  
  plot(x = 1:nrow(output), y = output$z_bar, type = 'l', ylab = expression("mean trait value "*italic(bar("z"))), xlab = "generation", main = paste("N = ", N, ", alpha = ", alpha, ", beta = ", beta, sep = ""))
...
output <- DemographyModel(N = 100, alpha = 30, beta = 15, t_max = 100)
...
output <- DemographyModel(N = 1, alpha = 30, beta = 15, t_max = 100)
...
DemographyModel2 <- function(N_max, alpha, beta, t_max) {
  ...
  output <- rep(0, N_max) # create output dataframe to hold change in mean trait value for each N 

  for (N in 1:N_max) {
  
    agent <- rgumbel(N, beta = beta)  # create first generation, n random draws from a gumbel with mode a=0 and dispersion beta
  
    for (t in 1:t_max) {
    
      z_bar <- sum(agent) / N  # current mean z
    
      z_h <- max(agent)  # get highest skill from current generation
    
      agent <- rgumbel(N, a = z_h - alpha, beta = beta)  # draw new values
    
      output[N] <- output[N] + sum(agent) / N - z_bar  # add new delta_z_bar to total delta_z_bar for this n
    
    }
  
    output[N] <- output[N] / t_max  # divide total delta_z_bar by t_max to get mean
...
Let's run this for $t_max = 50$ and $N_max = 5000$, and $alpha = 7$ and $beta = 1$. It might take a little while.

```{r}

output1 <- DemographyModel2(N_max = 5000, alpha = 7, beta = 1, t_max = 100)
...
plot(1:length(output1), output1, type = 'l', ylab = expression("change in "*italic(bar("z"))), xlab = "N")
abline(h = 0, lty = 2)
...
output2 <- DemographyModel2(N_max = 5000, alpha = 9, beta = 1, t_max = 100)

plot(1:length(output1), output1, type = 'l', ylab = expression("change in "*italic(bar("z"))), xlab = "N", col = "orange")
lines(output2, col = "blue")
abline(h = 0, lty = 2)
text(1300, 2, labels = expression("Simpler skill, "*alpha*"/"*beta*"=7"), col = "orange")
text(3000, -1.7, labels = expression("Complex skill, "*alpha*"/"*beta*"=9"), col = "blue")
...
plot(1:length(delta_z), delta_z, type = 'l', ylab = expression("change in "*italic(bar("z"))), xlab = "N", col = "orange")
abline(h = 0, lty = 2)
...
text(1300, 1.7, labels = expression("Simpler skill, "*alpha*"/"*beta*"=7"), col = "orange")
text(3000, -1.5, labels = expression("Complex skill, "*alpha*"/"*beta*"=9"), col = "blue")
