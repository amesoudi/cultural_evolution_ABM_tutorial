...
# Model 8: Vertical and horizontal cultural transmission
...
## Model 8a: Vertical cultural transmission
...
The following function takes the structure of **ConformistTransmission** from Model 4, replacing the three randomly-chosen demonstrators with two randomly-chosen parents, and setting the offspring traits as per the table above.
...
data_model8a <- VerticalTransmission(N = 10000, 
                                     p_0 = 0.1, 
                                     s_v = 0.0, 
                                     t_max = 150, 
                                     r_max = 5)
...
data_model8b <- VerticalAssortative(N = 10000, 
                                    p_0 = 0.01, 
                                    s_v = 0.1, 
                                    a = 0, 
                                    t_max = 150, 
...
## Model 8c: Horizontal cultural transmission
...
There are many ways of implementing horizontal cultural transmission, just like there are many ways of implementing oblique transmission, as we have seen in previous models (e.g. directly biased transmission, conformist biased transmission, blending inheritance). To allow a comparison with vertical transmission, here we will assume directly biased horizontal transmission. We will use a slightly different version of directly biased transmission from Model 3, modified to capture the key advantage of horizontal transmission over vertical transmission: the larger number of potential demonstrators. 
...
In Model 8c we assume that agents now choose $n$ members of the *same* generation, i.e. the set of agents who have already undergone vertical transmission. If at least one of those $n$ demonstrators has trait $A$, then the learner adopts trait $A$ with probability $s_h$.

This is directly biased transmission, as in Model 3, because $s_h$ allows agents with $B$ to switch to $A$. There is no possibility of agents with $A$ switching to $B$. The difference from Model 3 is that now there are $n$ demonstrators rather than one (similar to blending inheritance from Model 6), and we use the symbol $s_h$ to distinguish this selection parameter from the one incorporated into vertical transmission, $s_v$, and the one originally used in Model 3, $s$.
...
data_model8c <- VerticalHorizontal(N = 10000, 
                                   p_0 = 0.01, 
                                   s_v = 0.1, 
                                   s_h = 0, 
                                   a = 0.4, 
...
Increasing $n$ greatly increases the strength of selection due to horizontal cultural transmission. If you only need one out of 5 demonstrators to have $A$ for selection to operate, then selection for $A$ will be more frequent than if you need one out of one demonstrator to have $A$.
...
data_model8c_v <- VerticalHorizontal(N = 10000, 
                                     p_0 = 0.01, 
                                     s_v = 0.1, 
                                     s_h = 0, 
                                     a = 0.1, 
...
data_model8c_h <- VerticalHorizontal(N = 10000, 
                                     p_0 = 0.01, 
                                     s_v = 0, 
                                     s_h = 0.1, 
                                     a = 0, 
...
data_model8c_vh <- VerticalHorizontal(N = 10000, 
                                      p_0 = 0.01, 
                                      s_v = 0.1, 
                                      s_h = 0.1, 
                                      a = 0.1, 
...
for (r in 1:ncol(data_model8c_v)) {  
  lines(data_model8c_v[,r], type = 'l', col = "blue")  
}

# plot horizontal-only in orange
lines(rowMeans(data_model8c_h), type = 'l', lwd = 3, col = "orange")

for (r in 1:ncol(data_model8c_h)) {  
  lines(data_model8c_h[,r], type = 'l', col = "orange")  
}

# plot vertical-horizontal in grey
lines(rowMeans(data_model8c_vh), type = 'l', lwd = 3, col = "grey")

for (r in 1:ncol(data_model8c_vh)) {  
  lines(data_model8c_vh[,r], type = 'l', col = "grey")  
}
...
7. Rewrite **VerticalHorizontal**, replacing the horizontal cultural transmission rule with conformity, using code from Model 4. Does this change the conclusions regarding the speed of vertical vs horizontal transmission?
...
$$ p' = p + p(1-p)s_v   \hspace{30 mm}(8.1)$$
...
$$ p' = p + p(1-p)s_v(1-a)   \hspace{30 mm}(8.3)$$

This is the same as Equation 8.1 but with the difference between $p$ in successive generations also proportional to $(1-a)$. That is, the larger is $a$, the less change there is (at the extreme, $a = 1$, then $\Delta p = 0$). Assortative mating acts against selection.
...
If $p''$ denotes the frequency of $A$ after horizontal transmission, and $p'$ the frequency before horizontal transmission but after vertical transmission as per Equation 8.1 or 8.3, then:

$$ p'' = p' + (1-p')s_h(1 - (1-p')^n)    \hspace{30 mm}(8.4)$$
...
The potency of horizontal transmission lies in this last term. When $n = 1$, then $1 - (1-p)^n$ reduces to $p$, and we retrieve the same directly biased transmission form as Equations 3.1 and 8.1. As $n$ increases, then $1 - (1-p)^n$ tends to 1. Because $p$ must be less than or equal to 1, when $n > 1$ then horizontal transmission must be stronger than vertical transmission, for identical selection strength parameters, until an equilibrium is reached at $p = 1$ at which there is no change.

We can simulate the two recursions specified in Equations 8.3 and 8.4:
...
VerticalHorizontalRecursion <- function(s_v, a, s_h, n, t_max, p_0) {
  ...
  for (i in 2:t_max) {
    ...
    p[i] <- p[i-1] + p[i-1]*(1-p[i-1])*s_v*(1-a)  # Eq 8.3
    
    p[i] <- p[i] + (1-p[i])*s_h*(1-(1-p[i])^n)  # Eq 8.4
...
The equilibrium value $p^*$ can be found when $p = p''$. To make things easier, we can assume that $(1-p')^n$ in Equation 8.4 is approximately zero when $n$ is large, and so $(1 - (1-p)^n)$ is approximately 1. Removing this term, substituting Equation 8.3 into 8.4, and setting $p = p''$ gives:

$$ p = p + p(1-p)s_v(1-a) + s_h(1- (p + p(1-p)s_v(1-a))  )    $$

This can be rearranged to give:

$$ (1-p) (p (s_v(1-a) - s_h s_v(1-a)) + s_h) = 0  \hspace{30 mm}(8.5)$$
...
$$ p^* = -\frac{s_h} {(1-a) (s_v - s_h s_v)}    \hspace{30 mm}(8.6)$$

With the values used in the previous graph, we can use Equation 8.6 to find the value of the internal equilibrium that we observed:
...
Given that $p^*$ must be less than one, an internal equilibrium can only exist when the right hand side of Equation 8.6 is less than one. After rearranging, this gives the inequality:

$$ s_v (1-a) < -\frac {s_h} {(1-s_h)}    \hspace{30 mm}(8.7)$$

Because $s_h$ must be positive by assumption, the right hand side of Equation 8.7 must be negative. Consequently the inequality in Equation 8.7 can only be true when $s_v$ is negative, and moreover negative enough (after being reduced in strength by assortative cultural mating) to outweigh $s_h$ acting in the opposite direction.
