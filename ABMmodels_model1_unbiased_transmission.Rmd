...
The output of the model will be a plot showing $p$ over all generations up to the last generation, which we will call $t_{max}$. Generations (aka timesteps) are denoted by $t$, where generation one is $t = 1$, generation two is $t = 2$, up to the last generation $t = t_{max}$. These could correspond to biological generations, but could equally be 'cultural generations' (or learning episodes) within the same fixed population, which would be much shorter.
